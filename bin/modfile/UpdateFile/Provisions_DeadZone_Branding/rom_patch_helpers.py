...
def _apktool_dir() -> Path:
    return _project_root() / "DeadZone" / "Tools" / "Android" / "apktool"
...
def resolve_partition_root(work_dir: Path, partition_name: str) -> Path | None:
    candidates = [
        work_dir / partition_name / partition_name,
        work_dir / partition_name,
        work_dir / BUILD_IMAGES_REL / partition_name / partition_name,
        work_dir / BUILD_IMAGES_REL / partition_name,
    ]
    for candidate in candidates:
        if candidate.is_dir():
            return candidate
